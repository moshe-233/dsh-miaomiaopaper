window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULTS = {
		  ...
		  // - fabPosition: NAMED CORNER anchor used until the orb is dragged; picking
		  //   a corner in settings clears fabSnapY and returns to this anchor.
		  // - fabSnapY: vertical dock fraction ([0..1] of viewport height) ON THE
		  //   RIGHT EDGE. The orb does not free-move horizontally: dragging tracks
		  //   the finger vertically only, and release settles onto the right border
		  //   at that height (edge-magnet rail). null = follow the named corner.
		  fabEnabled: true,
		...
		// [local-patch] Validate a persisted right-edge dock height: an absolute
		// number in [0..1], or the legacy free-drag object ({x,y}) from which the Y
		// fraction migrates. Anything else falls back to the named corner.
		function normalizeFabSnapY(v) {
		  if (typeof v === "number") return Number.isFinite(v) ? Math.min(1, Math.max(0, v)) : null;
		  if (v && typeof v === "object" && Number.isFinite(Number(v.y))) {
		    return Math.min(1, Math.max(0, Number(v.y)));
		  }
		  return null;
		}
		...
		function sanitizeSettings(o) {
		  ...
		  return {
		    ...
		    fabSnapY: normalizeFabSnapY(o.fabSnapY !== undefined ? o.fabSnapY : o.fabPoint),
		...
		function syncFloatingOrb() {
		  ...
		  if (selection.fabSnapY != null) {
		    cls.push("we-fab--free");
		  } else {
		    cls.push("we-fab--" + (selection.fabPosition || "bottom-right"));
		  }
		  if (selection.fabMenuOpen) cls.push("we-fab--expanded");
		  ...
		  // rAF correction passes — nothing that can compound or race.
		  if (selection.fabSnapY != null) dockFabPosition(orb);
		}
		...
		function setupFabOutsideDismiss(orb) {
		  ...
		  // [local-patch] Centralized close: resets the drag module's click-eat flag,
		  // so a programmatically closed menu never swallows the user's next tap.
		  const closeMenu = () => {
		    selection.fabMenuOpen = false;
		    fabPointerSuppressClick = false;
		    syncFloatingOrb();
		    emit();
		  };
		  ...
		  const onTouchStart = (event) => {
		    ...
		    if (!(t instanceof Node) || orb.contains(t)) return;
		    closeMenu();
		  };
		  const onWindowBlur = () => {
		    if (!selection.fabMenuOpen) return;
		...
		// [local-patch] Free drag for the floating orb ─────────────────────────────
		// Right-edge rail: press-and-move the disc to slide the ball VERTICALLY
		// along the right border:
		//   * >6px travel turns a press into a DRAG (plain taps still toggle the menu)
		//   * the docked height is stored as fabSnapY (fraction of viewport height),
		//     re-clamped on resize so the ball never leaves the rail
		//   * picking any named corner in settings clears fabSnapY (back-compat)
		let fabDragCleanup = null;
		const FAB_EDGE_MARGIN = 12;     // min px between the ball and the top/bottom edges
		const FAB_DRAG_THRESHOLD = 6;   // px of travel before a press becomes a drag
		const FAB_RIGHT_INSET = 24;     // docked gap between the ball and the RIGHT border
		// Module-level so the outside-dismiss logic can reset it when it closes the
		// menu programmatically (otherwise one stale flag would eat the next tap).
		let fabPointerSuppressClick = false;

		function fabViewportSize() {
		...
		// Clamp a candidate viewport-top (px) for the docked ball against the
		// vertical borders; X is never free, so only Y needs clamping.
		function clampFabTopY(topPx) {
		  const { vw, vh } = fabViewportSize();
		  void vw;
		  return +Math.min(
		    Math.max(FAB_EDGE_MARGIN / vh, topPx / vh),
		    Math.max(FAB_EDGE_MARGIN / vh, (vh - 62 - FAB_EDGE_MARGIN) / vh),
		  ).toFixed(4);
		}

		// [local-patch] The ONLY writer of the free ball's inline position. X is
		// hard-pinned on the right border; Y comes from fabSnapY. The expanded
		// panel's side (below / above) is chosen from its REAL rendered height and,
		// in above mode, the container shifts ONCE, synchronously, so the BALL never
		// moves when the menu opens. No transitions anywhere on this path — the old
		// animated version raced its own measurements and flung the ball off-screen.
		function dockFabPosition(orb) {
		  if (selection.fabSnapY == null || typeof window === "undefined") return;
		  const vh = window.innerHeight || document.documentElement.clientHeight || 1080;
		  const margin = FAB_EDGE_MARGIN;
		  const size = 50; // ball diameter (.we-fab__trigger), incl. no menu offset
		  let ballTop = selection.fabSnapY * vh;
		  ballTop = Math.min(Math.max(ballTop, margin), Math.max(margin, vh - size - margin));

		  orb.style.left = "auto";
		  orb.style.right = FAB_RIGHT_INSET + "px";

		  // Real panel height NOW (content already rendered by the caller); headless
		  // mocks without offsetHeight degrade to 0 = default panel-below branch.
		  const menu = orb.querySelector(".we-fab__menu");
		  const mh = menu && Number.isFinite(menu.offsetHeight) ? menu.offsetHeight : 0;
		  const gap = 10; // .we-fab flex gap between ball and panel

		  if (selection.fabMenuOpen && mh > 0
		    && ballTop + size + gap + mh > vh - margin
		    && ballTop - mh - gap >= margin) {
		    // Panel above: normal column flow puts the PANEL at the container origin,
		    // so compensate once, synchronously — the ball stays visually pinned.
		    orb.classList.add("we-fab--free-above");
		    orb.style.top = (ballTop - mh - gap) + "px";
		  } else {
		    orb.classList.remove("we-fab--free-above");
		    orb.style.top = ballTop + "px";
		  }
		}

		function persistFabPoint() {
		  schedulePersist();
		}

		function setupFabDrag(orb) {
		  // [local-patch] Render churn safety: renderOrbContent() wipes orb.innerHTML
		  // and rebuilds the trigger button on EVERY sync, so listeners must live on
		  // the PERSISTENT orb container and resolve the disc per-event via
		  // closest('.we-fab__trigger') — the same reason the rope dock binds through
		  // a stable ref. Pointer capture is taken on the container itself so moves
		  // keep flowing even when the finger leaves the rebuilt node.
		  if (fabDragCleanup || typeof orb.addEventListener !== "function") return;
		  if (typeof orb.setPointerCapture !== "function") return;
		  // Minimal DOM hosts (headless verify mocks) lack window listeners.
		  if (typeof window === "undefined" || typeof window.addEventListener !== "function") return;

		  let activePid = null;   // pointerId currently held on the disc
		  let startX = 0, startY = 0, baseLeft = 0, baseTop = 0;
		  let dragging = false;
		  // suppressClick lives at module scope (fabPointerSuppressClick) so the
		  // outside-dismiss close path can clear it.

		  const onDown = (event) => {
		    if (event.pointerType === "mouse" && event.button !== 0) return;
		    const t = event.target;
		    if (!(t instanceof Element) || !t.closest(".we-fab__trigger")) return;
		    activePid = event.pointerId;
		    startX = event.clientX;
		    startY = event.clientY;
		    const rect = orb.getBoundingClientRect();
		    baseLeft = rect.left;
		    baseTop = rect.top;
		    // First drag migrates out of named-corner mode: seed the dock height from
		    // wherever the ball currently sits.
		    if (selection.fabSnapY == null) selection.fabSnapY = clampFabTopY(baseTop);
		    else baseTop = selection.fabSnapY * fabViewportSize().vh;
		    // NOTE: deliberately NO setPointerCapture on press. Capturing retargets
		    // the whole gesture — including the derived click — at the orb container,
		    // so the inner trigger button would never see plain taps and the quick
		    // menu could not open. Capture engages lazily once dragging starts (in
		    // onMove), the same shape as the rope dock whose surface hosts no child
		    // buttons.
		  };

		  const onMove = (event) => {
		    if (activePid !== event.pointerId) return;
		    const dx = event.clientX - startX;
		    const dy = event.clientY - startY;
		    if (!dragging && dx * dx + dy * dy < FAB_DRAG_THRESHOLD * FAB_DRAG_THRESHOLD) return;
		    if (!dragging) {
		      dragging = true;
		      selection.fabMenuOpen = false;
		      orb.classList.add("we-fab--dragging"); // instant follow: no settle transition
		      try { orb.setPointerCapture(event.pointerId); } catch { /* stale pid */ }
		    }
		    // RIGHT-RAIL MODE: X is locked for the entire gesture, not only after
		    // release. The ball can slide vertically, but never leaves the rail.
		    const { vh } = fabViewportSize();
		    selection.fabSnapY = clampFabTopY(baseTop + dy);
		    orb.style.left = "auto";
		    orb.style.right = FAB_RIGHT_INSET + "px";
		    orb.style.top = (selection.fabSnapY * vh) + "px";
		  ...
		  const finish = (event) => {
		    if (activePid !== event.pointerId) return;
		    activePid = null;
		    try { orb.releasePointerCapture(event.pointerId); } catch { /* released */ }
		    if (!dragging) return;
		    dragging = false;
		    fabPointerSuppressClick = true;
		    orb.classList.remove("we-fab--dragging");
		    dockFabPosition(orb);
		    persistFabPoint();
		  };

		  const onClickCapture = (event) => {
		    if (!fabPointerSuppressClick) return;
		    fabPointerSuppressClick = false;
		    event.preventDefault();
		  ...
		  // Belt-and-braces against native HTML5 drag hijacking the gesture (cover
		  // art yanked out of the disc): kill any dragstart born inside the orb.
		  const onDragStart = (event) => {
		    const t = event.target;
		    if (t instanceof Element && t.closest(".we-fab__trigger")) event.preventDefault();
		  };
		  ...
		  orb.addEventListener("click", onClickCapture, true);
		  ...
		  fabDragCleanup = () => {
		    ...
		    orb.removeEventListener("click", onClickCapture, true);
		    orb.removeEventListener("dragstart", onDragStart);
		    window.removeEventListener("resize", onResize);
		    fabDragCleanup = null;
		  };
		}
		function teardownFabDrag() {
		  if (fabDragCleanup) fabDragCleanup();
		}
		...
		function renderOrbContent(container) {
		  ...
		  if (coverUrl) {
		    ...
		    img.src = coverUrl;
		    img.alt = "";
		    // Native HTML5 image drag would hijack the pointer gesture and yank the
		    // cover art out of the disc — kill it at the source, not just via CSS.
		    img.draggable = false;
		  ...
		  return React.createElement("div", { className: "we-picker", "data-we-cards": sel.pickerLayout },
		    ...
		    React.createElement("div", { className: "we-picker__row" },
		      ...
		      sel.fabEnabled && React.createElement("select", {
		        className: "we-picker__playlist-select",
		        value: sel.fabSnapY != null ? "custom" : (sel.fabPosition || "bottom-right"),
		        onChange: (e) => {
		          const v = e.target.value;
		          if (v === "custom") { return; }
		          // Picking a named corner returns the orb to anchor mode: drop the
		          // right-rail dock height (it can be re-dragged at any time).
		          selection.fabPosition = v;
		          selection.fabSnapY = null;
		          persistSelection();
		          syncLayers();
		          emit();
		        },
		        title: "悬浮球固定角落；也可在主界面按住圆球沿右侧边框上下拖动吸附",
		      },
		      ...
		      React.createElement("option", { value: "custom", disabled: sel.fabSnapY == null }, "✦ 贴右缘吸附（拖拽后）"),
		      ),
		      React.createElement("span", { className: "we-picker__hint" },
		        sel.fabEnabled ? "一键切歌/播放/轮播；按住圆球沿右边框拖动，松手吸附在该高度" : "已隐藏悬浮球"),
		    ),
		...
		const CSS = `
		  ...
		  @media (prefers-reduced-motion: reduce) {
		    .we-rope--settle, .we-repo-panel, .we-picker__modal--panel, .we-repo-panel__modal-scrim,
		    .we-fab--free { transition: none !important; }
		  ...
		  /* [local-patch] Right-rail dock mode: inline top carries the docked height
		     while left is hard-pinned to the right border (X never varies). Geometry
		     contract (zero-shift): the orb container is column-reversed in FREE mode,
		     so the trigger button is ALWAYS the first/origin child — opening the menu
		     grows the panel away from the ball instead of pushing it around. The menu
		     normally hangs BELOW the ball; when a below-panel would leave the
		     viewport, --free-above flips the visual stack (panel above, one
		     correction frame nudges the anchor). */
		  .we-fab--free {
		    right: 24px; bottom: auto; left: auto; top: 0; flex-direction: column-reverse;
		    /* No position transition: menu open/close must not animate the rail pose. */
		    transition: none !important;
		  }
		  .we-fab--free-above { flex-direction: column; }
		  /* Hard vertical cap: the panel can NEVER grow taller than the viewport
		     (minus margins) — it scrolls internally, so no side-pick decision can
		     ever leave it stuck outside a border. */
		  .we-fab--free .we-fab__menu {
		    max-height: calc(100vh - 2 * 12px - 50px);
		    overflow-y: auto;
		    overscroll-behavior: contain;
		  }
		  .we-fab--dragging { transition: none !important; }
		  .we-fab--dragging .we-fab__menu { display: none; }

		  /* Main Floating Orb Trigger */
		  .we-fab__trigger {
		    position: relative; width: 50px; height: 50px; padding: 0; margin: 0;
		    border-radius: 50%; cursor: pointer;
		    /* [local-patch] touch-action:none lets pointer-drag work on pen/touch
		       without the page hijacking the gesture; grab cursor hints draggability. */
		    touch-action: none;
		  ...
		  /* [local-patch] Free-drag affordances: grab cursor in free mode, grabbing
		     while dragging, and the cover art can never start a native drag. */
		  .we-fab__disc img { -webkit-user-drag: none; user-select: none; }
		  .we-fab--free .we-fab__trigger { cursor: grab; }
		  .we-fab--dragging .we-fab__trigger { cursor: grabbing; transition: none !important; }
